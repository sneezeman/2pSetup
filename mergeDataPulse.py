...
def mergeDataPulse(filenameData):

    filenamePulse = filenameData.strip(filenameData.split('/')[-1]) + filenameData.split('/')[-1].replace(
        'Data',
        'Pulse')
    ...
    df.loc[idx2, 0] = df.loc[idx2, 0].apply(
        lambda x: float(x.split(':')[0]) * 3600 + float(x.split(':')[1]) * 60 + float(x.split(':')[2]))
    ...
    for i in range(df2[0].size):
        ...
        for k in range(number):
            ...
            dfToAdd[0] = dfToAdd[0].apply(
                lambda x: str(int(x // 3600)) + ':' + str(int((x % 3600) // 60)) + ':' + str(round(x % 60, 8)))
    ...
    outputFilename = filenameData.strip(filenameData.split('/')[-1]) + filenameData.split('/')[-1].replace('Data',
                                                                                                           'Merged')
...
if __name__ == "__main__":
    try:
        mergeDataPulse(sys.argv[1])
    except IndexError:
        filenameData = easygui.fileopenbox()
        # Check if user has selected pulse file
        if 'Pulse' in filenameData.split('/')[-1]:
            filenameData = filenameData.strip(filenameData.split('/')[-1]) + filenameData.split('/')[-1].replace(
                'Pulse', 'Data')
        mergeDataPulse(filenameData)
